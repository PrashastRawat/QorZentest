...
const StudentProtectedRoute = ({ children }) => {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f9f8f6', color: '#1c1917' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <p style={{ fontSize: '0.88rem', fontWeight: 700, color: '#78716c' }}>Verifying student classroom session...</p>
        </div>
      </div>
    );
  }
  ...
  if (!isAuthenticated) {
    // Redirect to login page and store attempted location in state
